...
# Functions
F_DISKS() {
  raw_space=$(fastfetch -s disk --logo none | while read line; do echo "${line#*:}"; done)
  raw_mount=$(fastfetch -s disk --logo none | while read line; do echo "${line#*k}" | grep -oP '.*(?=:)'; done)
  ...
  max=0
  for n in "${mount[@]}"; do
    if [[ $max -lt ${#n} ]]; then max=${#n}; fi
  done
  echo "$B         DISKS $G:" 
  for i in $(seq 0 $((${#space[@]}))); do
    str=$(printf "%$((max-${#mount[i]}))s")
    #space="for i in $(seq 0 $((max-${#mount[i]})) ); do echo -n " "; done)"
    echo "$W                $B${mount[i]}${str// / }$W ${space[i]}"
  done
}
